...
> socket path, wire format, field names, and watchdog mechanism may and will change
...
streams real-time VRAM memory state as NDJSON. Any external process can connect
to this socket and build automated remediation on top of Ingero's eBPF
observability.
...
```json
{"pid":12345,"allocated_bytes":8589934592,"total_vram":17179869184,"utilization_pct":50.0,"last_alloc_size":268435456,"timestamp_ns":1711180800000000000}
```
...
- One `MemoryState` is emitted per `cudaMalloc`, `cudaFree`, `cuMemAlloc_v2`,
...
while True:
    ...
    while b"\n" in buf:
        ...
        if state["utilization_pct"] > 90.0:
            print(f"WARNING: PID {state['pid']} at {state['utilization_pct']:.1f}% VRAM")
```
