...
func TestStepObservables_AccumulatesAndResets(t *testing.T) {
	...
	s.AddLaunch(k, 0, 100*time.Microsecond, now)
	s.AddLaunch(k, 0, 200*time.Microsecond, now)
...
func TestStepObservables_PerWorkloadIsolation(t *testing.T) {
	...
	s.AddLaunch(a, 0, 100*time.Microsecond, now)
...
func TestStepObservables_PruneStale(t *testing.T) {
	s := newStepObservables()
	t0 := time.Now()
	s.AddLaunch(observableKey{PID: 1}, 0, 100*time.Microsecond, t0)
	s.AddLaunch(observableKey{PID: 2}, 0, 100*time.Microsecond, t0.Add(2*time.Minute))
	s.AddLaunch(observableKey{PID: 3}, 0, 100*time.Microsecond, t0.Add(7*time.Minute))
...
func TestStepObservables_ConcurrentAdds(t *testing.T) {
	...
	for g := 0; g < goroutines; g++ {
		...
		go func(g int) {
			...
			for i := 0; i < iters; i++ {
				s.AddLaunch(k, 0, 1*time.Microsecond, now)
				s.AddMemcpy(k, 1, now)
			}
		}(g)
	}
...
func TestStepObservables_LastUpdateRefreshed(t *testing.T) {
	...
	s.AddLaunch(k, 0, 100*time.Microsecond, t0)
	s.AddLaunch(k, 0, 100*time.Microsecond, t1)
	got := s.Peek(k)
